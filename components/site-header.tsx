  ...
  return (
    <header className="sticky top-0 z-50 border-b border-border/70 bg-background/80 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <a href="/#inicio" className="flex shrink-0 items-center gap-2" aria-label="Codatra inicio">
        ...
        <nav className="hidden shrink-0 items-center gap-8 lg:flex" aria-label="Navegación principal">
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
        ...
        <button
          type="button"
          onClick={() => setOpen((v) => !v)}
          className="flex size-10 shrink-0 items-center justify-center rounded-lg text-foreground lg:hidden"
